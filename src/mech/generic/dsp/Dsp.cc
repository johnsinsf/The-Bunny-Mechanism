/*
  Author:  John Sullivan, Dynamic Processing Dsps LLC
...
const key_t Dsp::INITKEY2 = 0x16010000;
...
int
Dsp::main( DssObject& o ) {
  int       rc    = 0;
  _dpsServer = o.server;
  if( o.hostname.size() == 0 )
    o.hostname= get_hostname();

  logger.error("XXXN Dsp main running now " + itoa(geteuid()) + " localid " + o.localid + " on " + o.hostname + " dispatch_sem2 " + itoa(o.semRespID));
  ...
  Dsp* a = NULL;
  if( o.model == "DSPSHARE" || o.model == "dspshare" ) {
    logger.error("new dsp share host");
    a = new Dsp_share;
  } else {
