...
init_upnp (struct ushare_t *ut)
{
  ...
  if (ut->dlna_enabled)
  {
    ...
    description =
      dlna_dms_description_get (ut->name,
                                "Dwanta",
                                "https://buuna.dwanta.com/",
                                "bunnyuShare : DLNA Media Server",
                                ut->model_name,
                                "001",
                                "https://buuna.dwanta.com/",
                                "BUNNYUSHARE-01",
  ...
  log_info (_("Initializing UPnP subsystem ... %s\n"), ut->interface);
...
#define upnp_set_callback(cb, func) \
  ...
  if( ut->bunny ) {
    ...
    upnp_set_callback(Seek,    bunny_seek);
    upnp_set_callback(Write,   bunny_write);
    upnp_set_callback(Close,   bunny_close);
  } else {
    log_error (_("Using HTTP!\n"));  \
  ...
  UpnpSendAdvertisement (ut->dev, 100);
...
has_iface (char *interface)
{
  ...
  while (itf)
  {
    if ((itf->ifa_flags & IFF_UP)
        //&& !strncmp (itf->ifa_name, interface, IFNAMSIZ)) // TESTING!
        && !strncmp (itf->ifa_name, interface, 4 )) 
    {
      //log_error (_("Interface %s / %s is down.\n"), interface, itf->ifa_name);
      //log_error (_("Recheck uShare's configuration and try again !\n"));
      log_error (_("Interface %s / %s is up.\n"), interface, itf->ifa_name);
      freeifaddrs (itflist);
      return true;
    }
    itf = itf->ifa_next;
  ...
  for (i = n - 1 ; i >= 0 ; i--)
  {
    ...
    {
      /* interface is down */
      log_error (_("Interface1 %s is down.\n"), interface);
      log_error (_("Recheck uShare's configuration and try again !\n"));
      close (sock);
      return false;
    }
...
reload_config (int s __attribute__ ((unused)))
{
  ...
  if (ut2->xbox360)
  {
    ...
    name = (char*)malloc (strlen (XBOX_MODEL_NAME) + strlen (ut2->model_name) + 4);
...
{
  printf (_("%s (version %s), Internet UPnP A/V and DLNA Media Server.\n"),
          PACKAGE_NAME, VERSION);
  printf (_("John Sullivan (C) 2025, Dynamic Processing Systems LLC.\n"));
  printf (_("based on:\n"));
  printf (_("Benjamin Zores (C) 2005-2007, for GeeXboX Team.\n"));
  printf (_("See https://buuna.dwanta.com/ for more.\n"));
}
...
main (int argc, char **argv) {
  ...
  if( o.server != NULL ) {
    ...
    I = o.server->configMap.find("use_cache");
    if( I != o.server->configMap.end() ) {
      if( I->second == "no" )
        ut->use_cache = false;
    }
