...
{
  buffer_appendf (out, " %s=\"%zu\"", param, value);
}

static void
didl_add_item (struct buffer_t *out, int item_id,
               int parent_id, char *restricted, char *aclass, char *title,
               char *protocol_info, off_t size, char *url, char *filter)
{
  ...
  didl_add_tag (out, DIDL_ITEM_CLASS, aclass);
                    ...
                    char *title, char *aclass)
{
  ...
  didl_add_tag (out, DIDL_CONTAINER_CLASS, aclass);
